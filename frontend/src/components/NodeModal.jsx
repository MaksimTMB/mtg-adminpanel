...
export default function NodeModal({ node, onClose, onSave }) {
  ...
  const submit = async () => {
    if (!f.name || !f.host) { toast(t.nodeRequiredFields, 'error'); return; }
    setLoading(true);
    try {
      const payload = { ...f,
      ...
      if (node) { await api('PUT',  `/api/nodes/${node.id}`, payload); toast(t.nodeUpdated, 'success'); }
      else      { await api('POST', '/api/nodes', payload);            toast(t.nodeAdded,   'success'); }
      onSave();
  ...
  const checkAgent = async () => {
    ...
    try {
      ...
      toast(r.available ? `${t.nodeAgent}: ${t.nodeAgentAvailable}` : `${t.nodeAgent}: ${t.nodeAgentUnavailable}`, r.available ? 'success' : 'error');
  ...
  const updateAgent = async () => {
    ...
    toast(t.nodeAgentUpdating, 'info');
    try {
      const r = await api('POST', `/api/nodes/${node.id}/update-agent`);
      if (r.ok) toast(`${t.nodeAgent} ✓`, 'success');
  ...
  const copyInstallCmd = async () => {
    ...
    catch { toast(t.nodeAgentCopyError, 'error'); }
  };
    ...
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{maxWidth:520}}>
        <div className="modal-head">
          <div className="modal-title">{node ? <><I.Edit/> {t.editNodeTitle}</> : <><I.Plus/> {t.addNodeTitle}</>}</div>
          <button className="modal-close" onClick={onClose}><I.X/></button>
        </div>
        <div className="modal-body">
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <div className="form-group" style={{gridColumn:'1/-1'}}>
              <label className="form-label">{t.nodeNameLabel}</label>
              <input className="form-input" placeholder="Helsinki" value={f.name}
                onChange={e => set('name', e.target.value)} autoFocus/>
            </div>
            <div className="form-group" style={{gridColumn:'1/-1'}}>
              <label className="form-label">{t.nodeHostLabel}</label>
              <input className="form-input" placeholder="hel.maks68.com" value={f.host}
                onChange={e => set('host', e.target.value)}/>
            </div>
          </div>
          <div className="form-group">
            <label className="form-label">{t.nodeFlagLabel}</label>
            <FlagPicker value={f.flag} onChange={v => set('flag', v)}/>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <div className="form-group"><label className="form-label">{t.nodeSSHUserLabel}</label>
              <input className="form-input" placeholder="root" value={f.ssh_user} onChange={e => set('ssh_user', e.target.value)}/></div>
            <div className="form-group"><label className="form-label">{t.nodeSSHPortLabel}</label>
            ...
            <div className="form-group"><label className="form-label">{t.nodeBaseDirLabel}</label>
              <input className="form-input" placeholder="/opt/mtg/users" value={f.base_dir} onChange={e => set('base_dir', e.target.value)}/></div>
            <div className="form-group"><label className="form-label">{t.nodeStartPortLabel}</label>
          ...
          <div className="form-group">
            <label className="form-label">{t.nodeAuthLabel}</label>
            <div className="radio-group">
              <div className={`radio-btn ${auth === 'password' ? 'on' : ''}`} onClick={() => setAuth('password')}><I.Key/> {t.nodeAuthPassword}</div>
              <div className={`radio-btn ${auth === 'key' ? 'on' : ''}`} onClick={() => setAuth('key')}><I.Lock/> {t.nodeAuthKey}</div>
            </div>
          </div>
          {auth === 'password'
            ? <div className="form-group"><label className="form-label">{t.nodePasswordLabel}</label>
                <input className="form-input" type="password" placeholder="••••••••"
                  value={f.ssh_password || ''} onChange={e => set('ssh_password', e.target.value)}/></div>
            : <div className="form-group"><label className="form-label">{t.nodeKeyLabel}</label>
          ...
          <div style={{borderTop:'1px solid var(--b1)',paddingTop:16,marginTop:4}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
              <div style={{fontSize:13,fontWeight:600,display:'flex',alignItems:'center',gap:8}}>
                <I.Activity/> {t.nodeAgentTitle}
                {agentStatus === true  && <span className="badge badge-green" style={{fontSize:10}}>{t.nodeAgentAvailable}</span>}
                {agentStatus === false && <span className="badge badge-red"   style={{fontSize:10}}>{t.nodeAgentUnavailable}</span>}
                {node?.agent_port && agentStatus === null && <span className="badge badge-purple" style={{fontSize:10}}>:{node.agent_port}</span>}
              </div>
              <div style={{display:'flex',gap:8}}>
                {node && <button className="btn btn-ghost btn-sm" onClick={checkAgent} disabled={agentChecking}>
                  {agentChecking ? <><span className="spin spin-sm"/> {t.nodeAgentChecking}</> : <><I.Wifi/> {t.nodeAgentCheck}</>}
                </button>}
                {node && <button className="btn btn-secondary btn-sm" onClick={updateAgent} disabled={agentUpdating}>
                  {agentUpdating ? <><span className="spin spin-sm"/> {t.nodeAgentUpdating}</> : <><I.RefreshCw/> {t.nodeAgentUpdate}</>}
                </button>}
                <button className="btn btn-ghost btn-sm" onClick={() => setShowInstall(!showInstall)}>
                  <I.Download/> {t.nodeAgentInstall}
                </button>
              </div>
            </div>
            <div className="form-group">
              <label className="form-label">{t.nodeAgentPortLabel}</label>
            ...
            {showInstall && (
              <div style={{background:'var(--bg3)',borderRadius:10,padding:14,border:'1px solid var(--b1)'}}>
                <div style={{fontSize:12,color:'var(--t3)',marginBottom:8}}>{t.nodeAgentInstallHint} {f.host}:</div>
        ...
        <div className="modal-foot">
          ...
          <button className="btn btn-primary" onClick={submit} disabled={loading}>
            {loading ? <span className="spin spin-sm"/> : <><I.Check/> {node ? t.save : t.add}</>}
          </button>
        </div>
      </div>
    </div>
